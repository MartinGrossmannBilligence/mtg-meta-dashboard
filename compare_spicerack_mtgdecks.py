...
st, sp, sm = analyze_spicerack(spice_f)
...
print(f"{'Archetypes':<15} | {'N/A':<15} | {ma:<15}")
...
for d in [60, 90, 180, 365]:
    st, sp, sm = analyze_spicerack(get_spicerack_data(d))
    print(f"{d:>3} Days: Tournaments={st:<3}, Players={sp:<4}, Matches={sm:<4}")

# Compare with Duress crew 180 days (6 months)
ma6, mm6 = analyze_mtgdecks('data/archetype_matrix_6_months.json')
print(f"\nMTGDecks 180 Days (6 Months): Archetypes={ma6}, Matches={mm6}")
